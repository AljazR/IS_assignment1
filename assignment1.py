...
# Get the coordinates of 'S' and 'E': (x, y)
start_temp = list(zip(*np.where(maze == 2)))[0]
end_temp = list(zip(*np.where(maze == 3)))[0]
start = (start_temp[1], start_temp[0])
end = (end_temp[1], end_temp[0])

# Set parameters for GA
num_generations = 1000 # Number of generations.
num_parents_mating = 7 # Number of solutions to be selected as parents in the mating pool.
sol_per_pop = 50 # Number of solutions in the population.
gene_space = [0, 1, 2, 3, 4] # [up, down, left, right, stop]
...
def on_stop(ga_instance, fitness):
    ...
    # solution = [2,2,2,0,0,3,3,3,0,0,4,4,4,4,4] # Example of solution for maze2
    ...
    for i in solution:
        ...
        if (0 <= y and y < height) and (0 <= x and x < width):
            ...
            new_maze[y] = new_maze[y][:x] + 'X' + new_maze[y][x+1:]
